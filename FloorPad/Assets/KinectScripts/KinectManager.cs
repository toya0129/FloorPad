...
public class KinectManager : MonoBehaviour 
{
	...
	public static bool refreshTrigger = false;
	private bool textTrigger = true;
	private bool MusicTextTrigger = true;
...
//		}
	...
	void Awake()
	{
		LoadScript load = new LoadScript ();
		...
		if(!alUserIds.Contains(userId))
		{
			if(CheckForCalibrationPose(userId, bodyIndex, playerCalibrationPose))
			{
				...
				if (loadScript.nowSceneName == "TitleScene") {
					//テキスト変更
					if (textTrigger == true) {
						TenmetuScript.ChangeTrigger = true;
						textTrigger = false;
					}
				} else if (loadScript.nowSceneName == "PlayerReadScene") {
					//楽器読み込み
